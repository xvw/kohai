module Make (D : Generic.SIMPLE_RESOLVER) = struct
  ...
  let save ?id ~body (module H : Eff.HANDLER) item =
    let cwd = Global.ensure_supervision ?id ~body (module H) () in
    let file = D.resolver ~cwd in
    let items = list ?id ~body (module H : Eff.HANDLER) () in
  ...
  let get ?id ~body (module H : Eff.HANDLER) item =
    let items = list ?id ~body (module H : Eff.HANDLER) () in
    Kohai_model.Described_item.Set.find item items
  ;;

  let delete ?id ~body (module H : Eff.HANDLER) item_name =
    let cwd = Global.ensure_supervision ?id ~body (module H) () in
    let file = D.resolver ~cwd in
    let items = list ?id ~body (module H : Eff.HANDLER) () in
  ...
  let manip_counter f ?id ~body (module H : Eff.HANDLER) item_name =
    let cwd = Global.ensure_supervision ?id ~body (module H) () in
    let file = D.resolver ~cwd in
    let items = list ?id ~body (module H : Eff.HANDLER) () in
