...
let promote (module H : Eff.HANDLER) cwd log =
  ...
  State.upgrade (module H) cwd log
;;

let unpromote (module H : Eff.HANDLER) cwd log =
  let log_file = L.find_file ~cwd:(R.all_logs ~cwd) log in
  let content = Rensai.Lang.dump L.to_rensai log in
  let () = Eff.write_file (module H) log_file content in
  let () = unpropagate (module H) cwd log in
  let () = recompute_last_cache (module H) cwd log in
  State.downgrade (module H) cwd log
;;

let from_set (module H : Eff.HANDLER) set =
  let cwd = Global.ensure_supervision (module H) () in
  set |> Uuid.Set.to_list |> List.filter_map (get_by_uuid (module H) cwd)
;;

let get_list_by_cwd (module H : Eff.HANDLER) cwd =
  let now = Eff.now (module H) in
  let file = R.last_logs ~cwd in
  let content = Eff.read_file (module H) file in
  now, content |> Uuid.Set.from_file_content |> from_set (module H)
;;

let get_last (module H : Eff.HANDLER) () =
  let cwd = Global.ensure_supervision (module H) () in
  get_list_by_cwd (module H) cwd
;;

let get_last_for_sector (module H : Eff.HANDLER) sector =
  let cwd = Global.ensure_supervision (module H) () in
  let sector = Path.(R.sector_folder ~cwd / sector) in
  get_list_by_cwd (module H) sector
;;

let get_last_for_project (module H : Eff.HANDLER) project =
  let cwd = Global.ensure_supervision (module H) () in
